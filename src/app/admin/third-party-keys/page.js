...
export default function AdminThirdPartyKeys() {
  ...
  const testKey = async (service_name, api_key) => {
    try {
      const res = await fetch('/api/admin/third-party-keys/test', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
            ...
            <tbody>
              {keys.map((key, index) => (
                <tr key={key._id} className="border-b border-gray-50">
                  ...
                  <td className="p-4">
                    <button 
                      onClick={() => toggleStatus(key._id, key.is_active)}
                      className={`px-2 py-1 rounded text-xs font-medium transition-colors ${key.is_active ? 'bg-green-100 text-green-700 hover:bg-green-200' : 'bg-red-100 text-red-700 hover:bg-red-200'}`}
                    >
                  ...
                  <td className="p-4 text-right">
                    <button 
                      ...
                      className="text-blue-500 hover:text-blue-700 font-medium text-sm transition-colors mr-4"
                    >
                      Test
                    </button>
                    <button 
