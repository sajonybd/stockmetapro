...
export async function PATCH(request) {
  try {
    const { id, action, status, credit_limit, expire_date } = await request.json();
    ...
    if (action === 'toggle_status') {
      ...
      if (credit_limit) license.credit_limit = parseInt(credit_limit, 10);
      if (expire_date) license.expire_date = new Date(expire_date);
    }
